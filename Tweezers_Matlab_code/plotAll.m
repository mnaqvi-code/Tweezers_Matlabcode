...
if pltVr.grphAct == 2
    ...
    hold off
    %title(strrep(data.filename, '_', ' '));
    title({strrep(data.filename, '_', ' ');strcat('Time interval: ',selectedInterval)})
    xlabel('Time (s)')
...
if pltVr.grphAct == 3 %3 = Force-Extension
    kk = 1;
    ...
    set(gca,'Fontsize',16)
    %title(strrep(data.filename, '_', ' '));
    %title({strrep(data.filename, '_', ' ');graphTitleInterval})
    graphTitleInterval = strcat('Time interval:',32,num2str(round(data.x(1))),'-',num2str(round(data.x(length(data.x)))))
    title({strrep(data.filename, '_', ' ');graphTitleInterval})
...
hold off
saveas(gcf, fullfile(data.SaveFolder,strcat(datestr(now,formatTimePointOut),32,num2str(round(data.x(1))),'-',num2str(round(data.x(length(data.x)))))),'epsc');
saveas(gcf, fullfile(data.SaveFolder,strcat(datestr(now,formatTimePointOut),32,num2str(round(data.x(1))),'-',num2str(round(data.x(length(data.x)))))),'fig');
