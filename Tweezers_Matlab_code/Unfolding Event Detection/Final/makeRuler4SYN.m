...
T = 293.15; %K
Ld = (2113+798)*0.34*1E-9;%700.1E-9;%920E-9; %nm %500 basepairs for both dual DNA handles
Lp = 0.36*140*4E-9;%10E-9;%120E-9;%370*0.%120E-9; %nm
pP = 0.8E-9; %nm
pD = 50E-9; %nm
K = 1.5E-9;
P = pD;
...
L =  0.36*140*4E-9;%140*0.36E-9;%120*10^(-9);    % Ruud thesis: 120 nm  i.e. 0.33 per aa and 370 aa  / other references I have seen that people use 0.36 per aa 
%I2-M1 29 nm I2-M2 18
LpCalc3 = ([0 1 2 3]).*140.*0.36.*10^(-9); 
...
plot(x*1E6,force*1E12,(x+xprot1)*1E6,force*1E12,(x+xprot2)*1E6,force*1E12,(x+xprot3)*1E6,force*1E12,(x+xprot4)*1E6,force*1E12);