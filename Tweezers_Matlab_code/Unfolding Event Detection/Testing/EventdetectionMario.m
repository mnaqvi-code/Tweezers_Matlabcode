...
windowSize = 2; %Size of window in which to detect events
fSampling = 50; %Sampling frequency

for ii = 1:length(time)-windowSize
    forceMeanDiff(ii) = mean(diff(force(ii:ii+windowSize)));
    forceStd(ii) = std(force(ii:ii+windowSize));
end

forceStdTotal = std(force);

figure;
plot(time(1:end-windowSize),forceMeanDiff);

figure;
plot(time(1:end-windowSize),forceStd);

figure;
plot(time(1:end-windowSize),abs(forceMeanDiff)./forceStdTotal);

X = find(abs(forceMeanDiff)./forceStdTotal > 0.045);
...
for ii = 1:length(X)
    hold on;
    plot(time(X(ii)+1:X(ii)+2), force(X(ii)+1:X(ii)+2), 'k', 'LineWidth', 2);
end
hold off;
%plot(time(X), force(X), 'LineStyle', 'none', 'Marker', 'o');
