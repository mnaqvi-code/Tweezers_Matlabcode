function [x,nroot]=alirezacubicfcn(a, b, c, d)
% ----------------------------------------------------------------------
...
%           Every cubic equation could be simplified to:
%           y^3 + py + q = 0  
%           by dividing to a and replacing x = y - b/3a
%
...
%       1 real root & 2 conjugate complex roots OR 3 real roots (some are equal)
        ...
        u = abs(temp1)^(1./3.);           
        v = abs(temp2)^(1./3.);
        if (temp1 < 0.) u=-u; end       % Matlab can't use negative values as a base for powering?
        if (temp2 < 0.) v=-v; end
        y1  = u + v;
        y2r = -(u+v)/2.;
        y2i =  (u-v)*sqrt(3.)/2.;
...
% Step 4: Final transformation -----------------------------------------
      temp1 = b/a/3.;    % perform the back replacement of y to x
      y1 = y1-temp1;
      if (DD < 0.)
        y2 = y2-temp1;
        y3 = y3-temp1;
