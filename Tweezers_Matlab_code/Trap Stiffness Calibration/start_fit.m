...
clear n tolx fNyq T nblock channel channel_x channel_y channel_z want_alias want_hydro R l nu rho rhol want_filter elec_filters want_hist parametersX parametersY scal_fitX scal_fitY parameters0 Plot_start Plot_end Ffit_start Ffit_end;
cPos = 1;
nCal = 1;
global wndVr prgVr calData n tolx fNyq T nblock channel_x channel_y channel_z want_alias want_hydro R l nu rho rhol want_filter elec_filters want_hist parametersX parametersY scal_fitX scal_fitY parameters0 Plot_start Plot_end Ffit_start Ffit_end
...
if(size(wndVr.monPos,1) > 1)
    set(0,'defaultfigureposition',wndVr.monPos(size(wndVr.monPos,1)-1,:));
end;
...
hload_time_series  = uicontrol('Style','Pushbutton', ...
    ...
    'BackgroundColor',[1 0.1 0.1],...
    'Callback','extractDataOld');
hreturn_menu  = uicontrol('Style','Pushbutton', ...
    'Position',[left1+0.1*wid1 0.92 0.5*(wid1+wid2) height], ...
    'String','Return to Main Menu',...
    'FontSize',titlefs+0.2,...
    'BackgroundColor',[1 0.1 0.1],...
    'Callback','if(size(wndVr.monPos,1) > 1) set(0,''defaultfigureposition'',wndVr.monPos(size(wndVr.monPos,1),:)); end; tweezerStart');
if prgVr.setupStt == 1
