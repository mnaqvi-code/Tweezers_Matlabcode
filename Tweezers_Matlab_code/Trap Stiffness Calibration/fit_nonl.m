function [param, resnorm, r, J, x] = fit_nonl(f,param,Tolf,MaxIter,varargin)

%**************************************************************************
%Solves the problem:   min(sum(f(x,y...,param).^2)),  with respect to the parameters in vector 'param'
%                              param 
%[param, resnorm, r, J, x] = fit_nonl(f,param,Tolf,MaxIter,varargin)
%
%fit_nonl fits an inline expression or trial function 'f' to the data givin in
...
if nargin < 3, error('fit_nonl requires three input arguments');end
...
delta(find(p == 0)) = ones(length(p(find(p==0))),1)';
delta(find(p ~= 0)) = 1e-7*abs(p(find(p~=0)));
...
while (abs(f0-f1) > Tolf & iter < MaxIter)
    ...
    if (f1 < f0) %everything goes as planned, the function is decreasing
        if (f0-F == 0) check_trust = 1;
        else check_trust = (f0-f1)/(f0-F);
        end
        ...
        [pnew,f1,check] = linesearch(m,p,f0,gradf,-dp,norm(dp),f,varargin{:});
        pnew = pnew(:)';
        rnew = feval(f,pnew,varargin{:});
