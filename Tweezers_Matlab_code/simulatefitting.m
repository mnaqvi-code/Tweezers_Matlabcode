...
xlabel('Time (s)'); %set(h,'FontUnits','normalized','FontSize',0.04,'FontWeight','bold');
ylabel('Distance from trap center (\mum) '); %set(h,'FontUnits','normalized','FontSize',0.04,'FontWeight','bold');
...
hold off;
h = xlabel('Frequency (Hz)'); set(h,'FontSize',16,'FontWeight','bold');
h = ylabel(['P(f) (arbitrary units) ']); set(h,'FontSize',16,'FontWeight','bold');
set(gca,'XScale','log','YScale','log','FontSize',16,'FontWeight','bold');
title('Binned power spectrum with binsize = 512, Fsamp = 50 kHz, NSamp = 800k, NWindow = 32')
