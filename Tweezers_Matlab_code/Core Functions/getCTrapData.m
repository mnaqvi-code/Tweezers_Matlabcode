function [ConvertedData,ConvertVer,ChanNames,GroupNames,ci] = getCTrapData(varargin)
...
if ~isempty(data) %#ok<*NODEF>
    [filename,pathname] = uigetfile({'*.tdms','Raw Ctrap data files (*.tdms)'},'Pick data files',strrep(data.FileFolder, '\', filesep),'MultiSelect', 'off'); %#ok<*ASGLU>
else
    if exist('accessHistory.txt', 'file') == 2
        ...
        if exist(hisRead{1}{end,5},'dir') == 7
            [filename,pathname] = uigetfile({'*.tdms','Raw Ctrap data files (*.tdms)'},'Pick data files',hisRead{1}{end,5},'MultiSelect', 'off');
        else
            [filename,pathname] = uigetfile({'*.tdms','Raw Ctrap data files (*.tdms)'},'Pick data files',pwd,'MultiSelect', 'off');
        end
    else
        [filename,pathname] = uigetfile({'*.tdms','Raw Ctrap data files (*.tdms)'},'Pick data files',pwd,'MultiSelect', 'off');
    end
...
if isequal(filename,0)
    ...
    for fnum=1:numel(infilename)
        ...
        ConvertedData(fnum).pathname=FileNameLong;
...
ConvertedData.shiftX = 0; ConvertedData.shiftY = 0;
if strcmp(ConvertedData.Data.Root.Property(2).Value{1},'5')
    
    
    
elseif strcmp(ConvertedData.Data.Root.Property(2).Value{1},'6')
    
    
%    keyboard;
%     for gg = 1:length(ConvertedData.Data.MeasuredData(2).Property)
%         if length(ConvertedData.Data.MeasuredData(2).Property(gg).Name) >= 13
%             if strncmp('Bead Diameter',ConvertedData.Data.MeasuredData(2).Property(gg).Name,13)
%                 cFSplit = strsplit(ConvertedData.Data.MeasuredData(2).Property(gg).Value,'|');
%                 break;
%             end
%         end
%     end
%     breakVal = 0;
%     for gg = 1:length(ConvertedData.Data.MeasuredData.Name)
%         if strcmp('FD Data/Force Channel 0 (pN)',ConvertedData.Data.MeasuredData(gg).Name)
%             datVr.dualCst.cFreq1 = ConvertedData.Data.MeasuredData(gg).Property(3).Value;
%             breakVal = breakVal + 1;
%         end
%         if strncmp('FD Data/Force Channel 2 (pN)',ConvertedData.Data.MeasuredData(gg).Name)
%             datVr.dualCst.cFreq2 = ConvertedData.Data.MeasuredData(gg).Property(3).Value;
%             breakVal = breakVal + 1;
%         end
%         if strncmp('FD Data/Force Channel 1 (pN)',ConvertedData.Data.MeasuredData(gg).Name)
%             datVr.dualCst.cFreq3 = ConvertedData.Data.MeasuredData(gg).Property(3).Value;
%             breakVal = breakVal + 1;
%         end
%         if strncmp('FD Data/Force Channel 3 (pN)',ConvertedData.Data.MeasuredData(gg).Name)
%             datVr.dualCst.cFreq4 = ConvertedData.Data.MeasuredData(gg).Property(3).Value;
%             breakVal = breakVal + 1;
%         end
%         if breakVal >= 4
%             break
...
%     clear breakVal
    ...
    if ~isempty(find(cellfun(cellfind('Bead Diameter (um)'), {ConvertedData.Data.MeasuredData(2).Property.Name}), 1))
        if isstr(ConvertedData.Data.MeasuredData(2).Property(find(cellfun(cellfind('Bead Diameter (um)'), {ConvertedData.Data.MeasuredData(2).Property.Name}), 1)).Value)
        cFSplit = strsplit(ConvertedData.Data.MeasuredData(2).Property(find(cellfun(cellfind('Bead Diameter (um)'), {ConvertedData.Data.MeasuredData(2).Property.Name}), 1)).Value,'|');
        else
            cFSplit{1} = '2.1'; %um
            cFSplit{2} = '2.1'; %um
        end
    ...
    if exist('CTRAPCalValues.txt', 'file') == 2
        idx = max(strfind(ConvertedData.pathname,'\'));
            my_path = ConvertedData.pathname(1:idx);
            my_file_name = ConvertedData.pathname(idx+1:end);
            ConvertedData.FileInfo = dir(ConvertedData.pathname);     
        
        writeValAppend = dlmread('CTRAPCalValues.txt');
        timeMin = abs(writeValAppend(:,1)-ConvertedData.FileInfo.datenum);
        [idx idx] = min(timeMin);
        datVr.dualCst.cFreq1 =  writeValAppend(idx,2);
        datVr.dualCst.Dex1 = writeValAppend(idx,3); %Hz
        datVr.dualCst.cFreq2 =  writeValAppend(idx,4);
        datVr.dualCst.Dex2 = writeValAppend(idx,5); %Hz
        datVr.dualCst.cFreq3 =  writeValAppend(idx,6);
        datVr.dualCst.Dex3 = writeValAppend(idx,7); %Hz
        datVr.dualCst.cFreq4 =  writeValAppend(idx,8);
        datVr.dualCst.Dex4 = writeValAppend(idx,9); %Hz
        calLoadSuc = 1;
    end
    ...
    if calLoadSuc ~= 1
        if ~isempty(find(cellfun(cellfind('FD Data/Force Channel 0 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1))
            datVr.dualCst.cFreq1 = ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 0 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(3).Value; %Hz
        else
            datVr.dualCst.cFreq1 = 8000; %Hz
        end
    end
    
    datVr.dualCst.R1 = str2num(cFSplit{1}).*0.5E-6; %m
    ...
    if calLoadSuc ~= 1
        datVr.dualCst.alpha1 = abs(ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 0 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(1).Value)./ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 0 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(4).Value.*1E6; %um/a.u.
        datVr.dualCst.Dex1 =  datVr.dualCst.Dth1./(datVr.dualCst.alpha1./10^6).^2;
    else
        datVr.dualCst.alpha1 = sqrt(datVr.dualCst.Dth1/datVr.dualCst.Dex1)*10^6;
    end
    
    %Fit Parameters Trap 1 - Y
    if calLoadSuc ~= 1
        if ~isempty(find(cellfun(cellfind('FD Data/Force Channel 1 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1))
            datVr.dualCst.cFreq3 = ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 1 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(3).Value; %Hz
        else
            datVr.dualCst.cFreq3 = 8000; %Hz
        end
    end
    datVr.dualCst.R3 = str2num(cFSplit{1}).*0.5E-6; %m
    ...
    if calLoadSuc ~= 1
        datVr.dualCst.alpha3 = abs(ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 1 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(1).Value)./ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 1 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(4).Value.*1E6; %um/a.u.
        datVr.dualCst.Dex3 =  datVr.dualCst.Dth3./(datVr.dualCst.alpha3./10^6).^2;
    else
        datVr.dualCst.alpha3 = sqrt(datVr.dualCst.Dth3/datVr.dualCst.Dex3)*10^6;
    end
    
    %Fit Parameters Trap 2 - X
    if calLoadSuc ~= 1
        if ~isempty(find(cellfun(cellfind('FD Data/Force Channel 2 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1))
            datVr.dualCst.cFreq2 = ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 2 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(3).Value; %Hz
        else
            datVr.dualCst.cFreq2 = 8000; %Hz
        end
    end
    
    datVr.dualCst.R2 = str2num(cFSplit{2}).*0.5E-6; %m
    ...
    if calLoadSuc ~= 1
        datVr.dualCst.alpha2 = abs(ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 2 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(1).Value)./ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 2 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(4).Value.*1E6; %um/a.u.
        datVr.dualCst.Dex2 =  datVr.dualCst.Dth2./(datVr.dualCst.alpha2./10^6).^2;
    else
        datVr.dualCst.alpha2 = sqrt(datVr.dualCst.Dth2/datVr.dualCst.Dex2)*10^6;
    end
    %Fit Parameters Trap 2 - Y
    if calLoadSuc ~= 1
        if ~isempty(find(cellfun(cellfind('FD Data/Force Channel 3 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1))
            datVr.dualCst.cFreq4 = ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 3 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(3).Value; %Hz
        else
            datVr.dualCst.cFreq4 = 8000; %Hz
        end
    end
    datVr.dualCst.R4 = str2num(cFSplit{2}).*0.5E-6; %m
    ...
    if calLoadSuc ~= 1
        datVr.dualCst.alpha4 = abs(ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 3 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(1).Value)./ConvertedData.Data.MeasuredData(find(cellfun(cellfind('FD Data/Force Channel 3 (pN)'), {ConvertedData.Data.MeasuredData.Name}), 1)).Property(4).Value.*1E6; %um/a.u. 
        datVr.dualCst.Dex4 =  datVr.dualCst.Dth4./(datVr.dualCst.alpha4./10^6).^2;
    else
        datVr.dualCst.alpha4 = sqrt(datVr.dualCst.Dth4/datVr.dualCst.Dex4)*10^6;
    end
...
%%
    function [index SegInfo]=getChannelInfo(fid,SegInfo,NumOfSeg)
        ...
        set(h,'Name',['Converting TDMS File']);
        tTot = 0;
        tCount = 0;
        for segCnt=1:NumOfSeg
            ...
            if (kTocRawData)
                ...
                for kk=1:numel(objOrderList)
                    ...
                    if ccnt>0   %If segement has raw data
                        ...
                        %%%%%%%%%%%%%%%%HACK INCLUDED TO FIX MISSING
                        %%%%%%%%%%%%%%%%DATATYPE IN LUMICKS DATA
                        if ~isfield(index.(obname),'dataType')
                            index.(obname).dataType = 10;
                            index.(obname).datasize = 8;
                        end
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            tCount = tCount + 1;
            tTot = t + tTot;
            hours = floor((tTot/tCount)*(NumOfSeg-tCount)/3600);
            mins = floor(((tTot/tCount)*(NumOfSeg-tCount)-hours*3600)/60);
            secs = (tTot/tCount)*(NumOfSeg-tCount)-hours*3600-mins*60;
            waitbar((segCnt)/(NumOfSeg),h,[num2str(round(hours)) ':' num2str(round(mins)) ':' num2str(round(secs)) ' remaining.'])
        end
