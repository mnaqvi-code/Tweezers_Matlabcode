function [ data ] = plotFEdual( varargin ) % draws force-extension graphs for all chosen files
...
for ii = 1:size(data,2)    
    ...
    if pltVr.legOn == 1 && pltVr.splitPulls == 1
        ...
        C = distinguishable_colors(size(1:length(data.x),2));
    ...
    elseif pltVr.legOn == 1 && pltVr.splitPulls ~= 1
        ...
        C = distinguishable_colors(size(1:length(data.x)/2,2));
...
%             Pdev{jj} = Pdev{jj} -  ppOffset;
        ...
        elseif prgVr.forStt == 2 % Flinear
            ...
            xMir = data.data(pltVr.lIdx(jj):pltVr.rIdx(jj),11).*0.00016575398;%0.00016575398;%$0.00016575398;%0.000145;%0.00016575398;%0.00016575398 0.000123697
            %[Y,I] = sort(xMir);
            %xValU = unique(Y);
            %xPSD values.
            ...
            if ~exist('yDat1min')
                yDat1min = max(data.data(pltVr.lIdx(jj):pltVr.rIdx(jj),3));
            end
            if ~exist('yDat2min')
                yDat2min = min(data.data(pltVr.lIdx(jj):pltVr.rIdx(jj),7));
            end
            
            %yDat1 and yDat2 are swapped because approach is now from the right.
            yDat1 = (data.data(pltVr.lIdx(jj):pltVr.rIdx(jj),3)-yDat1min).*datVr.dualCst.k1.*datVr.dualCst.alpha1;
            yDat2 = -(data.data(pltVr.lIdx(jj):pltVr.rIdx(jj),7)-yDat2min).*datVr.dualCst.k2.*datVr.dualCst.alpha2;
            if ~exist('yDat')
