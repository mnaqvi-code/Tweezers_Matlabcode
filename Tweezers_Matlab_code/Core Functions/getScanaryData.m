function [ConvertedData,ConvertVer,ChanNames,GroupNames,ci] = getScanaryData(varargin)
...
global fluodata binaryD datVr;

if exist('fluodata') %#ok<*EXIST>
    
else
    fluodata = []; %#ok<*NASGU>
end

if ~isempty(fluodata) %#ok<*NODEF>
    [filename,pathname] = uigetfile({'*.tdms','Raw Scanary data files (*.tdms)'},'Pick data files',strrep(fluodata.FileFolder, '\', filesep),'MultiSelect', 'off'); %#ok<*ASGLU>
else
    if exist('accessHistory.txt', 'file') == 2
        ...
        if exist(hisRead{1}{end,5},'dir') == 7
            [filename,pathname] = uigetfile({'*.tdms','Raw Scanary data files (*.tdms)'},'Pick data files',hisRead{1}{end,5},'MultiSelect', 'off');
        else
            [filename,pathname] = uigetfile({'*.tdms','Raw Scanary data files (*.tdms)'},'Pick data files',pwd,'MultiSelect', 'off');
        end
    else
        [filename,pathname] = uigetfile({'*.tdms','Raw Scanary data files (*.tdms)'},'Pick data files',pwd,'MultiSelect', 'off');
    end
                    ...
                    if nvals>0  %If there is data in this segement
                        ...
                        switch id.dataType
                            ...
                            case 32		%String
                                ...
                                fluodata=cell(1,nvals*id.multiplier(rr));	%Pre-allocation
                                for mm=1:id.multiplier(rr)
                                    ...
                                    for dcnt=1:nvals
                                        ...
                                        fluodata{1,dcnt+(mm-1)*nvals}=char(convertToText(fread(fid,StrLength,'uint8=>char','l'))');
                                    end
                            ...
                            case 68		%Timestamp
                                ...
                                fluodata=NaN(1,nvals*id.multiplier(rr));
                                for mm=1:id.multiplier(rr)
                                    dn=fread(fid,2*nvals,'uint64',kTocEndian);
                                    tsec=dn(1:2:end)/2^64+dn(2:2:end);
                                    fluodata((mm-1)*nvals+1:(mm)*nvals)=tsec/86400+695422-4/24;
                                    fseek(fid,id.skip(rr),'cof');
                                end
                                %{
						for dcnt=1:nvals
                            ...
                            otherwise	%Numeric
                                ...
                                if (id.skip(rr)>0)
                                    ...
                                    [fluodata,cnt]=fread(fid,nvals*id.multiplier(rr),ntype,id.skip(rr),kTocEndian);
                                    if strcmp(matType,'uint8=>char')
                                        fluodata=convertToText(fluodata);
                                    end
                                else
                                    % Added by Haench start
                                    if  (id.dataType == 524300) || (id.dataType == 1048589) % complex CDB data
                                        [fluodata,cnt]=fread(fid,2*nvals*id.multiplier(rr),matType,kTocEndian);
                                        fluodata= fluodata(1:2:end)+1i*fluodata(2:2:end);
                                        cnt = cnt/2;
                                    else
                                        [fluodata,cnt]=fread(fid,nvals*id.multiplier(rr),matType,kTocEndian);
                                    end
                                    % Haench end
                        ...
                        if (cnt>0)
                            ob.(cname).data(ssamples+1:ssamples+cnt,1)=fluodata;
