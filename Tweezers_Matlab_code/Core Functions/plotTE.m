...
for jj =  1:length(data.x)/2       
    ...
    if pltVr.unfDet == 1;
        ...
        l = length(ext{jj});
        e1 = 0.006; % threshold for extension jumps defining start and end points [um]
        e2 = 0.0080; % threshold for step selection, minimal extension difference (between min and max in interval [start end]) [um]
        ...
        kk = 2;
        while kk <l-1
            if ext{jj}(kk+1)-ext{jj}(kk)> e1
               endpoint = kk;
               while ext{jj}(endpoint+1)-ext{jj}(endpoint)>0
                   endpoint = endpoint + 1;
